...
public class Player {
	...
	public synchronized void clicked(int x, int y) {
		int cx = car.getCurrentPos().getX() + car.getLastStep().getDeltaX(), cy = car
				.getCurrentPos().getY() + car.getLastStep().getDeltaY();
		int m = -1;
		if ((x == cx) && (y == cy))
			m = SAME;
	...
	public synchronized void move(int m) {
		...
		switch (m) {
		case LEFT:
			movecar(v.getDeltaX() - 1, v.getDeltaY());
			break;
		case RIGHT:
			movecar(v.getDeltaX() + 1, v.getDeltaY());
			break;
		case UP:
			movecar(v.getDeltaX(), v.getDeltaY() - 1);
			break;
		case DOWN:
			movecar(v.getDeltaX(), v.getDeltaY() + 1);
			break;
		case SAME:
			movecar(v.getDeltaX(), v.getDeltaY());
			break;
		}
	...
	public void moveai() {
		...
		level = 6 + (int) Math.round(car.getspeed() * 1.4); // Level is the
															// depth of the
															// search
		grass = (circ.terrain(car.getCurrentPos()) <= 0); // Is the car
															// currently at the
															// grass?

		Step v = car.getLastStep();
		distance = Math.atan2(-(cy - car.getCurrentPos().getY()), (cx - car
				.getCurrentPos().getX()))
				+ Math.PI; // Update the current distance of the car
		mv = ai(car.getCurrentPos().getX(), car.getCurrentPos().getY(),
				v.getDeltaX(), v.getDeltaY(), level);
		move((int) mv);
	}
